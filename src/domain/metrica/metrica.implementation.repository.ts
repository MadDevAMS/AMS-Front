...
import { MetricaMapper } from "./metrica.mapper";
...
export class MetricaImplementationRepository extends MetricaRepository {
  ...
  override updateMetrica(params: IMetricaModel): Observable<IApiResponse<void>> {
    return this.http.put<IApiResponse<void>>(`${API_URL}/metricas/${params.id}`, this.mapper.mapTo(params))
  }

  override createMetrica(params: IMetricaModel): Observable<IApiResponse<void>> {
    return this.http.post<IApiResponse<void>>(`${API_URL}/metricas`, this.mapper.mapTo(params))
  }

  override deleteMetrica(id: number): Observable<IApiResponse<void>> {
    return this.http.delete<IApiResponse<void>>(`${API_URL}/metricas?id=${id}`)
  }
}